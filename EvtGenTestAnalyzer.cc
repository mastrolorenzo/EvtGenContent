...
EvtGenTestAnalyzer::EvtGenTestAnalyzer( const ParameterSet& pset )
   : fOutputFileName( pset.getUntrackedParameter<string>("HistOutFile",std::string("TestBs.root")) ),
     tokenHepMC_(consumes<edm::HepMCProduct>(edm::InputTag(pset.getUntrackedParameter("moduleLabel",std::string("generator")),""))),
     fOutputFile(0)
{
}
...
void EvtGenTestAnalyzer::beginJob()
{
   ...
   hGeneralId = new TH1D( "hGeneralId","LundIDs of all particles",  2000, -1000., 1000.) ;
